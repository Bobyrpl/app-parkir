            ...
            <Table columns={['Username', 'Nama Akun', 'Catatan Pemohon', 'Diajukan', 'Status', 'Aksi']}>
                {data.map((p) => (
                    <tr key={p.id_permintaan}>
                        <td className="px-4 py-3 font-mono text-[#F4B400]">{p.username}</td>
                        <td className="px-4 py-3">
                            {p.user ? (
                                <>
                                    <p>{p.user.nama_lengkap}</p>
                                    <p className="text-xs text-[#8B94A3] capitalize">{p.user.role}</p>
                                </>
                            ) : (
                                <span className="text-xs text-[#8B94A3]">Akun tidak ditemukan</span>
                            )}
                        </td>
                        <td className="px-4 py-3 text-sm text-[#C3C9D3] max-w-xs">
                            {p.catatan || <span className="text-[#8B94A3]">—</span>}
                        </td>
            ...
            {!!tolakTarget && (
                ...
                >
                    <div
                        ...
                        aria-modal="true"
                    >
                        <h3 className="font-display text-base text-[#EDEFF2] mb-2">Tolak Permintaan Aktivasi</h3>
                        ...
                        <textarea
                            ...
                            className="w-full rounded-md bg-[#14181F] border border-white/10 px-3 py-2 text-sm text-[#EDEFF2] focus:outline-none focus:ring-2 focus:ring-[#F4B400] focus:border-transparent resize-none mb-4"
                        />
